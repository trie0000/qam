...
describe('license-chart', () => {
  ...
  it('prepareLicenseSeries: 年度ごとに 4月始まりの12ヶ月へ整列、無い月は null。指標で値を切替', () => {
    const samples = [s('2026-04-10T09-00-00', 80), s('2026-05-10T09-00-00', 90), s('2027-01-10T09-00-00', 95)];
    const series = prepareLicenseSeries(samples);
    expect(series.length).toBe(1);
    expect(series[0].fy).toBe(2026);
    expect(series[0].months[FY_MONTHS.indexOf(4)]).toBe(80);
    expect(series[0].months[FY_MONTHS.indexOf(5)]).toBe(90);
    expect(series[0].months[FY_MONTHS.indexOf(1)]).toBe(95);
    expect(series[0].months[FY_MONTHS.indexOf(6)]).toBeNull();
  });

  it('同年度・同月に複数サンプルがあれば最新(ts最大)を採用、年度は新しい順', () => {
    const series = prepareLicenseSeries([s('2025-04-01T09-00-00', 50), s('2026-04-01T09-00-00', 100), s('2026-04-20T18-00-00', 105)]);
  ...
  it('licenseChartSvg: SVG を生成し、表示年度のみ折れ線を描く', () => {
    const series = prepareLicenseSeries([s('2026-04-10T09-00-00', 100), s('2026-05-10T09-00-00', 110), s('2025-04-10T09-00-00', 50)]);
