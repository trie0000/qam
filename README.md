...
Qualys に **SCAN / MAP のスケジュールを新規作成**する（**作成のみ**。変更・削除は Qualys の画面で行う）。

- **SCAN** は v2 `/api/2.0/fo/schedule/scan/?action=create`、**MAP** は v1 `/msp/scheduled_scans.php?add_task=yes&type=map`。
  同じ意味の項目でも表記が違う（active が `0/1` と `yes/no`、曜日が小文字と先頭大文字、対象キーが
  `asset_groups` と `scan_target`）ため、内部表現から API ごとに変換している。
- 指定項目：種別／タイトル／対象（AssetGroup かドメイン・カンマ区切り、候補から選択可）／周期
  （毎日・毎週・毎月＋間隔・曜日・日）／開始日時／タイムゾーン／夏時間／スキャナー／
  オプションプロファイル／**作成時の状態（無効で作成 or 有効で作成）**。
- **本番への書き込み**なので、送信前に内容の要約を確認するモーダルを挟み、結果は操作履歴に残す。
  既定は「無効で作成」（Qualys 側で内容を確認してから有効化する運用）。
- **既定値は共通設定**で登録できる（SCAN のオプションプロファイル／MAP のオプションプロファイル／
  スキャナー（既定 `External`）／タイムゾーン（既定 `JP`））。登録画面に初期値として入り、その場で変更もできる。
- relay 側は送信先を上記 2 パスに限定している（任意 URL への POST は受け付けない）。
- **更新系 API の監査ログ**：スケジュール登録・ユーザ登録は、**実行者・発行した API（メソッドとURL）・
  パラメータ・結果**を `<QAM_DATA_DIR>/api-audit.log` に必ず出力する（認証情報は書かない）。参照系は対象外。
