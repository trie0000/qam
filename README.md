...
- **共通設定**：Qualys 接続先 POD、プロキシ URL、保存期間（日）、ライセンス上限、
  ...
  四半期検査の年度開始月・対象 AssetGroup パターン。
...
- **対象**：AssetGroup タイトルが対象パターン（既定 `^[A-Z]{2}[0-9]{3,4}D?$` ＝英字2＋数字3〜4＋末尾D任意）に
  一致するもの。SCAN は AssetGroup 単位、**MAP はその AG に登録されたドメイン単位**で判定する。
  **ドメイン未登録の AssetGroup は MAP 対象外**。
- **判定**：現四半期内に完了した実施があれば「検査済み」、無くても四半期内に次回実行予定がある
  有効なスケジュールがあれば「スケジュール済み」、どちらも無ければ**未対応**。
- **画面**：四半期の達成率カード（SCAN / MAP）→ **未対応の AssetGroup** → **週次サマリ**（その週の実施件数と
  累計カバレッジ）→ **対象×週マトリクス**（各対象がどの週に検査されたか）。
- **取得**：「Qualys から取得」で実施済み/スケジュールの scan・map を取得しキャッシュする（母集団の
