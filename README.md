...
「日付で貯めず、**変更で貯める**」。プログラムとデータは分離（データ場所は env）。

```
<QAM_DATA_DIR>/
  current/   group.json  host.json  domain.json     現在のフル状態（差分の相手・現況表示）
  history/   group.jsonl host.jsonl domain.jsonl     改廃イベント（追記のみ・履歴本体）
  comments/  comments.jsonl                          作業履歴コメント（追記のみ）
  runs.jsonl                                          取込メタ（日付/件数/completed）
  raw/<date>/{group,host,domain}.xml.gz              生XML（N日で剪定）
```
...
- [ ] **Phase 1** — XML アップロード取込 + 差分エンジン + 変更履歴ビュー + 資産一覧ビュー + フィルタ（参照）
