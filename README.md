...
- **画面**：四半期の達成率カード（SCAN / MAP）→ **対象母集団**（どの接続点が対象／対象外か、その理由）→
  **未対応の接続点** → **取得内訳（診断）** → **週次サマリ**（その週の実施件数と累計カバレッジ）→
  **対象×週マトリクス**（各対象がどの週に検査されたか）。
