...
- **資産種別**を最初に選ぶ。
  - **静的（IP 指定）**：検査種別を「MAP と SCAN の両方／MAP 単体／SCAN 単体」から選ぶ。
    検査資産情報は **IP のみ**（単体・CIDR・レンジ、複数指定可）。AssetGroup の `IP_SET` に登録される。
    **プライベートIP（10/8・172.16/12・192.168/16）は登録できない**（外部接続申請にもとづく検査は
    グローバルIP が前提のため、単体・CIDR・レンジの両端すべてを判定して弾く）。
  - **動的（FQDN 指定）**：検査種別は **SCAN のみ**（MAP 不可）。**IP の入力欄は無効化**され、
