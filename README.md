...
**AssetGroup とドメインを払い出してから、検査スケジュールを登録する**一連の操作をまとめて行う
（**作成のみ**。変更・削除は Qualys の画面で行う）。左ペインの独立ビューで、フォームはページ内に置く。
四半期検査ビューの「新規検査登録」ボタンからもこのビューへ移動する。

- **命名規則**：AssetGroup 名 = **「外部接続申請番号(仮)」**、ドメイン名 = **「小文字の申請番号 + "." + 地域コード」**
  （例 `EXT-2026-001` → AssetGroup `EXT-2026-001(仮)` / ドメイン `ext-2026-001.jp`）。
  `(仮)` は DNS 名に使えないためドメイン側では省く。英数字とハイフン以外はハイフンに落とす。
- **地域区分**（既定）：日本=`jp` / 北米=`na` / 中南米=`la` / 欧州・CIS・中東阿=`eu` / インド=`in` /
  中国・北東アジア=`cn`。**共通設定で追加・変更できる**（「ラベル=コード」のカンマ区切り）。
- **検査種別**：SCAN のみ / MAP のみ / 両方 から選ぶ。SCAN は AssetGroup 単位、MAP はドメイン単位。
- **SCAN 対象**：IP と DNS を入力し、AssetGroup の `IP_SET` / `DNS_LIST` に設定する。
  IP は行ごとに**「単体（IP か CIDR）」と「レンジ（開始〜終了）」を切り替え**られ、**複数行**指定できる。
  DNS も複数行。入力に応じて「作成される内容」がその場に表示される。
- **登録の流れ**：AssetGroup 作成 → （MAP のとき）ドメイン登録 → SCAN/MAP スケジュール登録。
  **同名の AssetGroup / ドメインが既にある場合は、「既存を使う / 別名で作る / 中止する」を確認する**
  （Qualys の AssetGroup 名は一意必須）。途中で失敗しても、そこまでに完了した内容を表示する。
