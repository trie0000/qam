...
UI は共通デザインルール（全アプリ共通）/ 開発基準（bookmarklet アプリ共通）に準拠。
...
**実装言語/構成（既存の内製ツールと同じ）:** アプリ本体は **TypeScript**（ブラウザ、esbuild バンドル、
...
- トークン: 共通トークン `tokens.css`（モスグリーン accent / paper 系）を `web/app.css` 先頭に同梱。
...
qam/
  src/                      ← TS アプリ本体（esbuild で qam.bundle.js に）
    ...
    styles/app.css          共通トークン同梱 + コンポーネント
