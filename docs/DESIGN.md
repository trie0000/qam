...
| エンティティ | スカラー | リスト/集合 |
|---|---|---|
| AssetGroup | TITLE, OWNER, LAST_UPDATE(参考) | **IPS（IP+範囲）, DNS_LIST**, NETBIOS_LIST, DOMAIN_LIST, HOST_IDS(取得可能なら) |
| Host | IP, DNS/FQDN, NETBIOS, OS, TRACKING_METHOD, LAST_VULN_SCAN | （所属 AssetGroup は参考情報） |
| Domain | （ドメイン=キー） | NETBLOCK |
