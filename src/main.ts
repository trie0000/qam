...
async function openQuickInspectModal(initial?: ProvisionInput): Promise<void> {
  ...
  const [cfg, registry] = await Promise.all([getConfig(), loadRegistry()]);
...
async function loadRegistry(): Promise<AssetRegistry> {
  const src: { group?: RegistrySource; domain?: RegistrySource; host?: RegistrySource } = {};
  for (const e of ['group', 'domain', 'host'] as const) {
    const stamps = await getSnapshotStamps(backend, e);
    const stamp = stamps[stamps.length - 1];
    if (!stamp) continue;
    const snap = await readSnapshot(backend, e, stamp);
    if (snap) src[e] = { stamp, records: snap.records };
  }
  return buildRegistry(src);
}

// トラッキング方式の食い違い警告。理由を出したうえで、「検査担当に確認済み」に
// チェックが入るまで登録させない（既定は未チェック＝続行不可）。
async function confirmTracking(issues: AssetCheck[]): Promise<boolean> {
  const body = el('div', {}, [
    el('div', { style: 'margin-bottom:var(--s-4)' }, [
      callout('検査対象の資産が、host list には別のトラッキング方式で登録されています。'),
    ]),
    el('div', { class: 'qam-track-list' }, issues.map((c) =>
      el('div', { class: 'qam-track-item' }, issueLines(c).map((t) => el('div', {}, [t]))))),
    el('div', { class: 'qam-track-note' }, [TRACKING_CONFIRM_NOTE]),
  ]);
  const cb = el('input', { type: 'checkbox' }) as HTMLInputElement;
  body.append(el('label', { class: 'qam-prov-opttoggle' }, [cb, el('span', {}, ['検査担当に登録可否を確認済み'])]));
  return new Promise((resolve) => {
    let done = false;
    openModal({
      title: 'トラッキング方式が登録内容と異なります',
      body,
      primaryLabel: 'この内容で続行',
      dismissBackdrop: false,
      onPrimary: () => {
        if (!cb.checked) { toast('検査担当に確認済みのチェックを入れてください', 'error'); return false; }
...
async function runProvision(
  ...
  try {
    const agTitle = plan.title;
    // 1) AssetGroup。同名があれば作り直さず、その AssetGroup を更新する。
    //    IP/DNS は add_* で足す（set_* は上書きなので、以前の登録分を消してしまう）。
    const found = await findAssetGroup(creds, agTitle);
    if (found) {
      await editAssetGroup(creds, buildAssetGroupEditParams(p, found.id), author);
      steps.push(`AssetGroup「${agTitle}」を更新（既存に追加）`);
    } else {
      await createAssetGroup(creds, buildAssetGroupParams(p), author);
      steps.push(`AssetGroup「${agTitle}」を作成`);
    }
    // 2) ドメイン（MAP 対象がある場合のみ・複数あり得る）。同名は更新する。
    //    edit は netblock 必須かつ送った内容が正になるので、既存 + 今回分をマージして送る。
    const domains: string[] = [];
    for (const d of plan.domains) {
      const cur = await findDomain(creds, d);
      if (!cur) {
        await writeDomain(creds, buildDomainParams(p, d), author);
        steps.push(`ドメイン「${d}」を登録`);
      } else {
        const merged = mergeNetblocks(cur.netblocks, plan.netblocks);
        if (merged.added.length) {
          await writeDomain(creds, buildDomainEditParams(d, merged.list), author);
          steps.push(`ドメイン「${d}」を更新（ネットブロック追加: ${merged.added.join(', ')}）`);
        } else steps.push(`ドメイン「${d}」は登録済み（変更なし）`);
      }
      domains.push(d);
    }
    // 3) スケジュール
...
function confirmModal(title: string, message: string, primaryLabel = '取り込む'): Promise<boolean> {
  return new Promise((resolve) => {
    let done = false;
    openModal({
      // 改行を保つ（複数行の確認内容が1行に潰れて読めなくなるのを防ぐ）。
      title, body: el('div', { class: 'qam-confirm-msg' }, [message]), primaryLabel,
...
function openHelp(): void {
  const body = el('div', { class: 'qam-help', html: `
    ...
    <ul>
      ...
      <li><b>簡易検査</b>（左ペイン）：開くと<b>登録履歴の管理表</b>が表で出ます。上部の<b>「＋ 新規登録」</b>で
          登録モーダルを開き、<b>行クリックでその内容をプリフィルして再登録</b>できます（検査予定日は本日に戻ります）。
          <b>AssetGroup（とドメイン）を払い出してから、検査スケジュールを登録</b>します
          （作成のみ。変更・削除は Qualys の画面で行ってください）。
          まず<b>資産種別</b>を選びます。<b>静的（IP 指定）</b>は検査種別を「両方／SCAN のみ／MAP のみ」から選べ、
          検査資産情報として IP を行ごとに<b>「単体（IP・CIDR）」と「レンジ（開始〜終了）」で切り替え</b>て複数指定します。
          <b>動的（FQDN 指定）</b>は SCAN のみで、検査資産情報として FQDN を複数指定します（<b>IP は入力できません</b>）。
          静的の IP には<b>プライベートIP（10/8・172.16/12・192.168/16）は登録できません</b>。
          ...
          <b>同名の AssetGroup／ドメインが既にある場合は、作り直さず「更新」で登録</b>します
          （IP・DNS は追加、ドメインは既存ネットブロックに今回分を足す。以前の登録内容は消しません）。
          入力中は<b>取り込み済みデータと突き合わせ</b>て、同名が既にあれば「作成される内容」の下に
          <b>赤字</b>で更新になる旨を出し、資産ごとに <b>新規</b>（host list 未登録＝検査時に新規登録される）／
          <b>既存・IP追跡</b>／<b>既存・DNS追跡</b>のバッジと「host list へ新規登録される見込み」を表示します
          （host を取り込んでいないと「判定不可」になります）。
          <b>FQDN 登録なのに既存が IP トラッキング</b>、<b>静的IP 登録なのに既存が DNS トラッキング</b>の場合は
          理由付きの警告が出て、<b>「検査担当に登録可否を確認済み」にチェックを入れるまで登録できません</b>。
