...
# ─── 社内イントラ / Outlook（共有部品）─────────────────────────────────────
# 実装は intra-mail.ps1（アプリ非依存）。ここでは認証情報の復号とログだけを足す。
. (Join-Path $PSScriptRoot 'intra-mail.ps1')
$script:QamLog = { param([string]$t) Add-QamLog $t }

function Invoke-QamIntraFetch { param($Body)
    # DPAPI の暗号文はここで平文に戻してから渡す（共有部品は復号を知らない）。
    $b = $Body | Select-Object *
    $b | Add-Member -NotePropertyName psw -NotePropertyValue (Get-QamPassword $Body) -Force
    return Invoke-IntraFetchFile $b $script:QamLog
}
function Invoke-QamOutlookDraft { param($Body)
    $r = Invoke-OutlookDraft $Body $script:QamLog
    if ($r.ok) { Add-QamAudit $Body.author 'OUTLOOK' 'draft' ("to=" + [string]$Body.to) '下書きを表示' }
    return $r
}
...
function Invoke-Route { param($Ctx)
    ...
    switch -Regex ($path) {
        ...
        '^/qam/intra/fetch-file$' {
            if ($req.HttpMethod -ne 'POST') { Send-Json $Ctx @{ error = 'POST のみ' } 405; return }
            try { Send-Json $Ctx (Invoke-QamIntraFetch (Get-Body $req | ConvertFrom-Json)) }
            catch { Send-Json $Ctx @{ ok = $false; error = $_.Exception.Message } 500 }
            return
        }
        '^/qam/outlook/draft$' {
            if ($req.HttpMethod -ne 'POST') { Send-Json $Ctx @{ error = 'POST のみ' } 405; return }
            try { Send-Json $Ctx (Invoke-QamOutlookDraft (Get-Body $req | ConvertFrom-Json)) }
            catch { Send-Json $Ctx @{ ok = $false; error = $_.Exception.Message } 500 }
            return
        }
