...
function Invoke-QamIntraFetch { param($Body)
    ...
    try {
        ...
        $form.Add((New-Object 'System.Collections.Generic.KeyValuePair[String,String]'('psw', (Get-QamPassword $Body))))
        $content = New-Object System.Net.Http.FormUrlEncodedContent($form)
        Add-QamLog "INTRA login POST $loginUrl"
        ...
        # 4) 本体
        Add-QamLog "INTRA download $fileUrl"
...
function Invoke-QamOutlookDraft { param($Body)
    ...
    try {
        ...
        Add-QamAudit $Body.author 'OUTLOOK' 'draft' ("to=" + [string]$Body.to) '下書きを表示'
